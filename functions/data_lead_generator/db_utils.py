	...
	for x, key in enumerate(columns):
		...
		if ('timestamp' in data_type and value != None):
			try:
				...
				print('[{},{}] Unable to parse datetime for column {}'.format(record['account_id'], record['account_lead_id'], key))
				value = None
			...
			try:
				...
				print('[{},{}] Unable to parse date for column {}'.format(record['account_id'], record['account_lead_id'], key))
				value = None
...
def recursive_insert(records, stage_table_name, table_name, cur, conn, unique_columns, defaults, mapping, update, chunk = None):
	...
	while not successful and count < 2:
		try:
			...
			print("::::: [{}]{} Data set length: {}".format(table_name, chunk_num, len(records)))
			...
			conn.commit()
			if (isinstance(e, UnicodeError) or isinstance(e, psycopg2.errors.SyntaxError) or isinstance(e, psycopg2.errors.DataException) or isinstance(e, psycopg2.errors.NumericValueOutOfRange)):
				if(len(records) > 25):
					print("::::: [{}]{} DB ERROR -> Splitting records in half".format(table_name, chunk_num))
					...
					print("::::: [{}]{} DB ERROR -> Running {} records one by one in order to find bad record".format(table_name, chunk_num, len(records)))
					bad_counter = 0
					for i, item in enumerate(records):
						try:
							...
							cur.execute(sq)
							conn.commit()
							print("::::: [{},{}]{} Successfully inserted record".format(table_name, i, chunk_num))
						except Exception as err:
							cur.execute('rollback;')
							conn.commit()
							acc_id = item['account_id'] if 'account_id' in item else 'Unkown'
							print("::::: [{}][Account: {}] Found bad data; Inserting into bad_data table".format(table_name, acc_id))
							print("::::: [{}][Account: {}] {}".format(table_name, acc_id, err.pgerror))
							bad_counter += 1
					...
					print("::::: [{}]{} Successfully upserted {} records. Failures: {}".format(table_name, chunk_num, len(records)-bad_counter, bad_counter))
	...
	if (successful):
		print("::::: [{0}]{1} Rows upserted".format(table_name, chunk_num))
	else:
...
def insert_to_db(records, table_name, cur, conn, unique_columns, defaults={}, mapping=[], update=True):
	try:
		if len(records) > 0:
			print("::::: [{1}] Retrieved {0} records".format(len(records), table_name))	
			print("::::: [{0}] Unique columns: {1}".format(len(records), ', '.join(unique_columns)))	
			unique_records = remove_duplicates(records, unique_columns) 
			print("::::: [{0}] Removed {1} records".format(table_name, len(records)-len(unique_records)))

			if (update):
				print("::::: [{0}] Upserting {1} records".format(table_name, len(unique_records)))
			else:
				print("::::: [{0}] Inserting {1} records".format(table_name, len(unique_records)))
		...
		print("[{}] ----> DB error: ".format(table_name) + str(err))
		conn.rollback()
		stack = traceback.format_exc()
		print(err, str(stack), str(sys.exc_info()))
