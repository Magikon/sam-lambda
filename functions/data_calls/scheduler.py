...
def update(event, context):
	try:
		...
		for account_row in data:
			...
			try:
				triggers = account_config['calls_producer']['triggers']
			except KeyError:
				err = "Unable to find configuration in DynamoDB: <calls_producer.triggers>"
				rollbar_error = ProPairError(err, error_type="CallsScheduler")
			...
			if (run_incremental):
				...
				response = sns.publish(
					TopicArn=os.environ["TOPIC_DATA_ARCH_CALLS_REDSHIFT"], 
					Subject="Calls Incremental",   
					Message=json.dumps(message, default=decimal_default)
				)
				secure_log(response)
			else:
				secure_log("::::: Calls Incremental set to false for account {}. Skipping...".format(message['account']))
		...
		rollbar_error = ProPairError(err, error_type="CallsScheduler", exec_info=sys.exc_info(), stack=stack)
		report_to_rollbar(os.environ["TOPIC_ROLLBAR"], rollbar_error)
		conn.rollback()
