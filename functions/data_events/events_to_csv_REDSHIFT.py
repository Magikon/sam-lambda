...
key_id = current_credentials.access_key
...
def update(event, context):
	...
	try:
		...
		if response != None:
			...
			if leads['Leads'] != None and 'Lead' in leads['Leads']:
				...
				if (isinstance(leads['Leads']['Lead'], list)):
					for lead in leads['Leads']['Lead']:
						...
						if (os.environ["ENV"] == "staging" and bool(random.random() < 0.1) and 'Agent' in lead):
			...
			csv_file = insert_to_csv(normalized_events, cur, conn, account, start_date,end_date)
			upload_to_s3(account, csv_file)
			upload_to_RS(account,cur,conn,csv_file)
...
def upload_to_RS(account,cur,conn,csv_file):
	secure_log("::::: Copy CSV to Redshift")

	sql = """
			copy external_events 
				from 's3://pp-tables-prod/events-tmp2/{}/{}'
				...
				csv
				gzip;
			""".format(account['name'].lower(),csv_file,key_id,secret_key)
...
def insert_to_csv(records, cur, conn, account, start_date, end_date):
	...
	try:
		if len(records) > 0:
			...
			csv_file = "{}_{}_to_{}.csv.gz".format(account['name'], start_csv_date, end_csv_date)

			insert['events'].to_csv(csv_file, index=False, sep="|", compression='gzip')

			#try:
			#	with open(csv_file, 'w') as csvfile:
			#		writer = csv.DictWriter(
			#		csvfile, 
			#		fieldnames=insert['csv_columns'].keys(),
			#		delimiter="|"
			#		)
			#		writer.writeheader()
...
def get_insert_statement(records, cur, conn):
	...
	query = "SELECT \"column\",type from pg_table_def where tablename='external_events';"
	cur.execute(query)
	data = cur.fetchall()
	...
	for line in data:
		if line[0] not in ignored_columns:
			csv_columns.append({"name": line[0], "type": line[1]})
	...
	for i, record in enumerate(records):
		...
		for x, a in enumerate(csv_columns):
			key = a['name']
	...
	for i in csv_columns:
		...
		if col in list(events_df.columns):
			if ('time' in dtype):
				print("::::: Setting {} to datetime".format(col))
				events_df[col] = pd.to_datetime(events_df[col])
			if ('int' in dtype):
				print("::::: Setting {} to int".format(col))
				events_df = events_df.astype({col: 'Int64'})

	#rearrange columns
	events_df = events_df[[ x['name'] for x in csv_columns]]

	return {'events': events_df, 'csv_columns': csv_columns}
