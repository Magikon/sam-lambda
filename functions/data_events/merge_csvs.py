...
def main():
    ...
    try:
        ...
        for o in all_customers['CommonPrefixes']:
            ...
            #export to csv
            combined_csv.to_csv(location+".gzip", index=False, encoding='utf-8-sig', compression='gzip')
            secure_log("Successfully merged csv files for {}".format(customer))
            secure_log("Location: {}".format(location))

            # config s3 for multipart 
            GB = 1024 ** 3
            config = TransferConfig(multipart_threshold=5 * GB)   

            #upload to s3
            s3u = boto3.resource('s3')
            s3u.meta.client.upload_file(location, os.environ["BUCKET"], 'external_events/{}/{}'.format(customer,file_name), Config=config)
