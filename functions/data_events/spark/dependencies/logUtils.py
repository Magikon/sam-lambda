...
def get_aws_credentials(config):
    # get secrets 
    secret_name = "{}/aws_credentials".format( config["user"] )

    # Get AWS Credentials from secret mgr
...
def get_account_config(id, refresh = False):
    ...
    if ((accounts_configurations == None) or (refresh == True)):
        print("::::: Getting Account Configuration")
        config_table = dynamodb.Table('customer-configuration')
        ...
        def unflatten(data):
            ...
            for k, v in data.items():
                ...
                for key in matches:
                    ...
                    if prop in cur or (0 <= prop < len(cur) and cur[prop] != None):
                        cur = cur[prop]
                    elif key[1] != '':
                        cur[prop] = []
                        cur = cur[prop]
